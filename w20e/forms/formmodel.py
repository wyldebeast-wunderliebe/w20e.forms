...
class FormModel(object):
    ...
    def _eval(self, expression, context, function_registry):
        if self._expressionlanguage == 'javascript':
            return evaluator.eval_javascript(expression, context, function_registry)

        return evaluator.eval_python(expression, context, function_registry)
    ...
    def isRelevant(self, field_id, data):
        ...
        for props in self.getFieldProperties(field_id):
            if props.getRelevant() is not None:
                try:
                    if not self._eval(
                        props.getRelevant(), {"data": data}, Registry.funcs
                    ):
                        return False
                except:
                    logger.exception(
    ...
    def collectEfferentFields(self):
        ...
        for prop in list(self._props.values()):
            for rule in [
                ...
                try:
                    self._eval(getattr(prop, rule, ""), {"data": Collector(prop.bind)})
                except:
